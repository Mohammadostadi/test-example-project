...
//  plz update i\d with index !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  data.forEach((el, index) => {
    el.id = String(index)
    const html = `<li class="non-active-products" id="${el.name}"><a href="#">${
      el.name
    }</a></li>`;
    menuContainer.insertAdjacentHTML("beforeend", html);
    arrayCategories.push(document.getElementById(`${el.name}`));
  });
...
const showProducts = function (product) {
  ...
  data.then((data) => {
    ...
    if(data.total == 0){
      const html =`<div class="justify-content-center align-items-center d-flex flex-column">
      <div class="div-text">
                <h1>404</h1>
